...
OFFICIAL  = re.compile(r'\.(gov\.in|nic\.in|npci\.org\.in|uidai\.gov\.in|rbi\.org\.in)$', re.I)
...
def _analyze(message: str) -> dict:
    ...
    susp_urls = [u for u in urls if not OFFICIAL.search(u.split("?")[0])]
...
def analyze():
    ...
    if len(message) > 5_000:
        return jsonify({"error": "Message too long. Max 5,000 characters."}), 400
